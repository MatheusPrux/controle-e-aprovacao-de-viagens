...
const DashboardAdmin: React.FC<DashboardAdminProps> = ({ trips, userRole, onUpdateStatus }) => {
  ...
  // Ambas as roles visualizam viagens com status 'Pendente'
  ...
  const handleAction = (tripId: string, status: TripStatus) => {
    ...
    onUpdateStatus(tripId, status, comment, {
      numero_dt: finance.dt,
      valor_comissao: Number(finance.val.replace(',', '.'))
    });
  };

  const isFormValid = (tripId: string) => {
    const finance = activeFinance[tripId];
    return finance && finance.dt.trim() !== '' && finance.val.trim() !== '';
  };
    ...
    <div className="space-y-8 animate-fade-in">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-6">
        <div>
          ...
          <p className="text-gray-500 font-bold mt-2">Nível: <span className="text-indigo-600 uppercase">{userRole}</span> • Visualizando pendências para validação</p>
        </div>
      ...
      <section className="space-y-10">
        {filteredTrips.length === 0 ? (
          <div className="bg-white p-24 rounded-[3rem] border-2 border-dashed border-gray-100 text-center text-gray-300">
             ...
             <p className="font-black uppercase tracking-widest text-[10px]">Tudo limpo por aqui.</p>
          </div>
        ) : (
                   ...
                   <div className="space-y-4">
                      <div className="grid grid-cols-2 gap-4">
                         ...
                         <div>
                            ...
                            <input 
                              type="text" 
                              value={activeFinance[trip.id]?.val || trip.valor_comissao?.toString() || ''} 
                   ...
                   <div className="flex flex-col h-full justify-between">
                      <div className="bg-indigo-900 rounded-[2rem] p-6 text-white mb-6">
                         <div className="flex justify-between items-center">
                            ...
                            <span className="text-indigo-400 font-black">{(trip.kmFinal || 0) - trip.kmInitial} KM</span>
                         </div>
                      ...
                      <div className="flex gap-4">
                        ...
                        >
                          {trip.status === 'Aprovado' ? 'Atualizar Dados' : 'Aprovar e Validar'}
                        </button>
                      </div>
                   </div>
                </div>
